...
const INTEREST_GROUPS_FILE = getDataPath("interest_groups.json");
...
async function loadGroups() {
  try {
    ...
    console.log(
      `✅ Loaded ${Object.keys(interestGroups.groups).length} interest groups`
    );
  } catch (err) {
...
async function saveGroups() {
  try {
    await fs.writeFile(
      INTEREST_GROUPS_FILE,
      JSON.stringify(interestGroups, null, 2),
      "utf8"
    );
    return true;
  } catch (err) {
...
function getAllGroups() {
  return Object.values(interestGroups.groups).sort(
...
function getGroupsByPhone(phone) {
  return Object.values(interestGroups.groups).filter((group) =>

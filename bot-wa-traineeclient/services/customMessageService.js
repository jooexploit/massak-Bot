...
const CUSTOM_MESSAGES_FILE = getDataPath("custom_messages.json");
const SCHEDULED_MESSAGES_FILE = getDataPath("scheduled_messages.json");
...
async function deleteMessage(id) {
  ...
  if (usedInSchedule) {
    throw new Error(
      `Cannot delete: message is used in schedule "${
        usedInSchedule.name || usedInSchedule.id
      }"`
    );
  }
...
function validateMessage(content) {
  ...
  if (openBrackets !== closeBrackets) {
    errors.push(
      "عدد الأقواس غير متطابق - تأكد من إغلاق جميع المتغيرات {{...}}"
    );
  }

...
function detectCategoryFallback(text) {
  ...
  // ⚠️ CRITICAL: Detect if this is a REAL ESTATE ad before checking Hasak categories
  // This prevents real estate ads with keywords like "استراحة", "شاليه", "محل", "كوفي", "مطعم"
  // from being incorrectly routed to Hasak instead of Masaak
  ...
  const hasRealEstateContext = realEstateIndicators.some((kw) =>
    text.includes(kw),
  );
  ...
  const categoryKeywords = {
    ...
    // ⚠️ These are SKIPPED if real estate context is detected (hasRealEstateContext)
    "حراج الحسا": [
      "حراج الحسا",
      "حراج الأحساء",
      "حراج الحساء",
    ...
    "كوفيهات أو مطاعم": [
      // ⚠️ Only match when NOT a real estate ad (e.g., "محل كوفي للبيع" should go to Masaak)
      "كوفيهات أو مطاعم",
      "كوفيهات ومطاعم",
      "افتتاح كوفي",
      "افتتاح مطعم",
    ...
    "منتجعات وإستراحات": [
      // ⚠️ Only match when NOT a real estate ad (e.g., "استراحة للبيع" should go to Masaak)
      "منتجع",
      "منتجعات",
      "حجز استراحة",
      "حجز شاليه",
    ...
    "محلات تجارية": [
      // ⚠️ Only match when NOT a real estate ad (e.g., "محل للبيع" should go to Masaak)
      "محلات تجارية",
      "افتتاح محل",
      "عروض محل",
      "تخفيضات",
  ...
  // List of Hasak-specific categories that should be SKIPPED when real estate context is detected
  const hasakOnlyCategories = [
    "كوفيهات أو مطاعم",
    "منتجعات وإستراحات",
    "محلات تجارية",
    "مركز ترفيهي",
  ...
  for (const [category, keywords] of Object.entries(categoryKeywords)) {
    // ⚠️ CRITICAL: Skip Hasak categories that overlap with real estate keywords
    // when the text clearly contains real estate indicators
    if (hasRealEstateContext && hasakOnlyCategories.includes(category)) {
      console.log(
        `🏷️ Skipping Hasak category "${category}" because real estate context detected`,
      );
      continue;
    }
...
3. الفئات والفئات الفرعية:
  ...
  🎪 للفعاليات والأنشطة والخدمات - فئات حساك (Hasak):
   ...
   ⚠️ **هام جداً - التفريق بين حساك ومسعاك لنفس الكلمات**:
   
   الكلمات التالية يمكن أن تذهب لمسعاك أو حساك حسب السياق:
   
   🔹 "استراحة" / "شاليه":
   - إذا كان الإعلان عن بيع أو إيجار عقار (يحتوي على: للبيع، للإيجار، متر، صك، عرق، مشاع، مساحة، غرف) → مسعاك (Masaak) - arc_category = "استراحة" أو "شالية"
   - إذا كان الإعلان عن حجز استراحة لمناسبة أو إيجار يومي أو سياحة → حساك (Hasak) - arc_category = "منتجعات وإستراحات"
   
   🔹 "كوفي" / "مطعم" / "كافيه":
   - إذا كان الإعلان عن بيع أو إيجار أو تقبيل محل (يحتوي على: للبيع، للإيجار، للتقبيل، متر، مساحة) → مسعاك (Masaak) - arc_category = "تجاري" أو "محل تجاري"
   - إذا كان الإعلان عن افتتاح كوفي/مطعم جديد أو عروض أو قائمة طعام → حساك (Hasak) - arc_category = "كوفيهات أو مطاعم"
   
   🔹 "محل" / "محلات":
   - إذا كان الإعلان عن بيع أو إيجار أو تقبيل محل عقاري (يحتوي على: للبيع، للإيجار، للتقبيل، متر، مساحة) → مسعاك (Masaak) - arc_category = "محل تجاري" أو "تجاري"
   - إذا كان الإعلان عن محل يقدم عروض أو خدمات أو افتتاح → حساك (Hasak) - arc_category = "محلات تجارية"
   
   🔹 القاعدة العامة: إذا الإعلان يتكلم عن عقار (بيع/شراء/إيجار عقار) → مسعاك. إذا يتكلم عن خدمة أو فعالية أو نشاط تجاري → حساك.

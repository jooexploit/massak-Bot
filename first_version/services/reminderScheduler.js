...
// CRITICAL: Import socket manager for live socket access (optional fallback to direct socket)
let socketManager = null;
try {
  socketManager = require("../whatsapp/socketManager");
} catch (err) {
  console.log("ℹ️ socketManager not available, using direct socket fallback");
}

// Fallback socket reference when socketManager is not set
let fallbackSocket = null;

function getActiveSocket() {
  if (socketManager?.getSocket) {
    const live = socketManager.getSocket();
    if (live) return live;
  }
...
function registerTimeout(id, timeoutId) {
  if (socketManager?.registerTimeout) {
    socketManager.registerTimeout(id, timeoutId);
  }
}

function unregisterTimeout(id) {
  if (socketManager?.unregisterTimeout) {
    socketManager.unregisterTimeout(id);
  }
}

function registerInterval(id, intervalId) {
  if (socketManager?.registerInterval) {
    socketManager.registerInterval(id, intervalId);
  }
}

function unregisterInterval(id) {
  if (socketManager?.unregisterInterval) {
    socketManager.unregisterInterval(id);
  }
}

function registerCronJob(id, task) {
  if (socketManager?.registerCronJob) {
    socketManager.registerCronJob(id, task);
  }
}

function unregisterCronJob(id) {
  if (socketManager?.unregisterCronJob) {
    socketManager.unregisterCronJob(id);
  }
}
...
async function sendReminderWithRetry(reminder, maxRetries = 3) {
  ...
  if (!isConnected()) {
    console.log(
      `⚠️ Connection not ready for reminder ${reminder.id}, will retry later`
    );
    return false;
  }
  ...
  if (timeUntil < 60 * 60 * 1000) {
    const timeoutId = setTimeout(async () => {
      ...
      if (!isConnected()) {
        ...
        unregisterTimeout(`reminder_${reminder.id}`);
        return;
      }
    ...
    registerTimeout(`reminder_${reminder.id}`, timeoutId);
  } else {
    ...
    try {
      ...
      // CRITICAL: Register with socket manager for cleanup on disconnect
      registerCronJob(`reminder_${reminder.id}`, task);
    } catch (error) {
      console.error(
...
function cancelScheduledReminder(reminderId) {
  ...
  if (job) {
    if (job.type === "timeout") {
      clearTimeout(job.id);
      unregisterTimeout(`reminder_${reminderId}`);
    } else if (job.type === "cron" && job.task) {
      job.task.stop();
      unregisterCronJob(`reminder_${reminderId}`);
    }
...
function initScheduler(sockInstance) {
  // Store socket if socketManager is not used
  fallbackSocket = sockInstance || fallbackSocket;

  console.log(
    "✅ Initializing smart reminder scheduler (socket-aware with fallback)..."
  );
  ...
  // Register with socket manager for cleanup (no-op if unavailable)
  registerInterval("reminder_backup_check", backupCheckInterval);
...
function stopScheduler() {
  ...
  if (backupCheckInterval) {
    clearInterval(backupCheckInterval);
    unregisterInterval("reminder_backup_check");

...
// Data files
const WASEET_FILE = dataSync.getFilePath("WASEET_CONTACTS");
...
function loadWaseetContacts() {
  try {
    ...
    const data = dataSync.readDataSync("WASEET_CONTACTS", { waseet: [] });
    waseetContacts = data;
  } catch (error) {
    console.error("❌ Error loading waseet contacts:", error);
    waseetContacts = { waseet: [] };
  }
}
...
function saveWaseetContacts() {
  try {
    dataSync.writeDataSync("WASEET_CONTACTS", waseetContacts);
  } catch (error) {
